...
public class MessagesController(IMessageRepository messageRepository, IMemberRepository memberRepository) : BaseApiController
{
    ...
    public async Task<ActionResult> DeleteMessage(string id)
    {
        ...
        if (message == null) return  BadRequest("Cannot delete message that doesn't exist.");

        if (message.SenderId != memberId && message.RecipientId != memberId) return BadRequest("You are not authorized to delete this message.");
